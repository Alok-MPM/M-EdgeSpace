...
function parse(s){s=s.toLowerCase();
 ...
 if(/(lamba karo)/.test(s))return{op:'stretch',axis:'x',f:1.3};
...
function runLocal(txt){const c=parse(txt);
 ...
 if(c.op==='help'){FX.toast('🤖 "cube lao" "sphere banao" "sheet banao" "ghumao 45" "copy karo" "paste karo" "idea note karo ..."');return}
 if(c.op==='idea'){FX.toast('📜 idea saved ('+IDEAS().length+')');say('Idea note kar liya');return}
 if(c.op==='ideas'){FX.toast('📜 '+IDEAS().length+' ideas stored');return}
